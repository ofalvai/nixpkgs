...
, libjpeg, pixman, cairo, pango, which, postgresql, giflib, cctools }:
...
in myNodePackages.package.override {
  ...
  nativeBuildInputs = [ node-pre-gyp nodejs.pkgs.node-gyp-build pkg-config which ] ++ lib.optionals stdenv.hostPlatform.isDarwin [ cctools ];
  buildInputs = [ libjpeg pixman cairo pango postgresql ] ++ lib.optionals stdenv.hostPlatform.isDarwin [ giflib ];
